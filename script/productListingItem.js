...
const AuctionItem = require("../models/productListingItem");
...
program
  .command("seed")
  .description("Seed the database with items from db_test.json")
  .action(async () => {
    await connectToDB();
    try {
      const data = JSON.parse(
        fs.readFileSync(path.join(__dirname, "../db/db_test.json"))
      );
